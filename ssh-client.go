...
func NewSshClient(user,hostname,port,passwd string ,key *Key) (*ssh.Client, error) {
	var err error
	var addr string
	...
	if key != nil {
		...
		if passwd!="" {
			singer, err = ssh.ParsePrivateKeyWithPassphrase([]byte(key.PrivateKey),[]byte(passwd))
		}else {
